...
class Neural_Network:
    ...
    def Read_Files(self):
        ...
        for i in range(8):
            ...
            while j <=120:
                ...
                # print(str(file) +"\n")
                #make different files for different transformations
                shift_img=np.roll(file,3,axis=1)
                shift_img2=np.roll(file,-3,axis=1)
                rows,cols = file.shape[:2]
                rotation_matrix = cv2.getRotationMatrix2D((cols/2,rows/2),45,1)
                rotation_matrix2 = cv2.getRotationMatrix2D((cols/2,rows/2),-45,1)
                
                rotation_img = cv2.warpAffine(file,rotation_matrix,(cols,rows))
                rotation_img2 = cv2.warpAffine(file,rotation_matrix2,(cols,rows))
                cv2.imwrite("KangHong/rotate/Img " + "(" + str(j) + ").jpg",rotation_img)
                cv2.imwrite("KangHong/rotate1/Img " + "(" + str(j) + ").jpg",rotation_img2)
                imgs=[file,shift_img,rotation_img,rotation_img2,shift_img2]
    ...
    def Saving_Weights_Bias(self,filename="KangHongBo_Q2_Weights.txt"):
        """
...
if __name__ == "__main__":
    nn=Neural_Network(784,200,15)
    inputs, outputs = nn.Read_Files()
    epochs = 1000
    threshold = 0.001
    for i in range(epochs):
        E_total = nn.Training_Neural_Network(inputs, outputs,learn_rate=1)
