...
class Neural_Network:
    ...
    def Read_Files(self):
        ...
        for i in range(8):
            ...
            while j <=120:
                ...
                #resize to 7*7
                # file = cv2.resize(file,(7,7))
                reshaped_image = file.reshape(7,4,7,4)
    ...
    def Saving_Weights_Bias(self,filename="KangHongBo_Exp2_Weights.txt"):
        """
...
def Load_Weights_Bias(filename="KangHongBo_Exp2_Weights.txt.npz"):
...
for i in range(1,31):
    ...
    # np.save('resized_image.npy', pooled_image)
    # print(str(file) +"\n")
    #prepare kernels - Sobel
    kernel_x = np.array([[-1,-2,-1],[0,0,0],[1,2,1]])
    kernel_y = np.array([[-1,0,1],[-2,0,2],[-1,0,1]])
