...
class ArraySortedList(SortedList[T]):
    ...
    def _set_key(self, item: ListItem, key) -> None:
        """
        Function: Sets the key for the item to be sorted.

        Time complexity: O(1) since there is only item assignment.
        """
        item.key = key

    def _add(self, item: ListItem, key: str) -> None:
        """ 
        Function: Add new element to the list. 
        
        Complexity: O(comp(>=)). The line self.is_full() is the dominant line which has time complexity of O(comp(>=)). 
        The other lines of code are only item assignments and mathematical operations, which are in O(1).
        """
        if self.is_full():
            self._resize()

        self._set_key(item, key)
        # find where to place it
        position = self._indextoadd(item)
