...
class ArraySortedList(SortedList[T]):
    ...
    def __setitem__(self, index: int, value: ListItem) -> None:
        """ Magic method. Insert the item at a given position,
            if possible (!). Shift the following elements to the right.
        """
        if self.is_empty() or \
                (index == 0 and value.key <= self[index].key) or \
                (index == len(self) and self[index - 1].key<= value.key) or \
                (index > 0 and self[index - 1].key <= value.key <= self[index].key):
    ...
    def find(self,key)->list:
        """
        Finds an item in the array by key.

        Complexity: O(n) where n is the length of self.array. The worst case is when we traverse through
        the whole list (when the item is in the last position or there isn't any item found in the array).
        """
        for i in self.array:
            if i.key==key:
                return i
