...
class BinarySearchTree(Generic[K, I]):
    ...
    def delete_aux(self, current: TreeNode, key: K) -> TreeNode:
        ...
        if current is None:  
            ...
            # General case (all cases )leads to succession
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            Complexity: O(log(n)), where n is the number of nodes in the tree. Since it only traverses 
            a part of the tree (traverses to current.right, then calls get_minimal, which is in O(log(n) 
            as well) in each method call, the total complexity is only in O(log(n)).
        """
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        """
            ...
            Complexity: O(log(n)), where n is the number of nodes in the tree. Since it only traverses 
            a part of the tree (keeps traversing left until it hits the end) in each method call, the total 
            complexity is only in O(log(n)). 
        """
